...
try:
    ...
    if not current_url or settings.database_url:
        database_url = settings.database_url or "postgresql+asyncpg://postgres:password@localhost:5432/yieldarbitrage"
        config.set_main_option("sqlalchemy.url", database_url)
except Exception:
    # If we can't access config (e.g., during import testing), just skip URL override
    pass
...
async def run_async_migrations() -> None:
    ...
    This function creates an async engine using the configuration
    and runs migrations within an async context.
    """
    # Get database configuration
    alembic_config = config.get_section(config.config_ini_section, {})
    
    # Create async engine with optimal settings for migrations
